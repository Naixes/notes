...
- 类型系统实际上是最好的文档，接口定义**可以直接生成文档**，大部分的函数看看类型的定义就可以知道如何使用了
- **类型检查**，可以在编译阶段就发现大部分错误，也可以**方便V8进行优化**
...
<img src="TypeScript.assets/截屏2021-06-05 上午8.12.27.png" alt="截屏2021-06-05 上午8.12.27" style="zoom:50%;" />

js：程序运行时动态计算属性偏移量；额外的空间存储属性名；所有对象的偏移量信息各存一份

c++：编译阶段确定偏移量；偏移量替代属性名访问；偏移量信息共享

### js与V8

parse词法分析语法分析 - 解释器 - 字节码 - turbofan编译器，jit编译 - 机器码，将字节码替换成机器码

解释 ｜ 优化（去优化）

使用ts，方便v8优化，在优化时，代码热检查，频繁执行函数，type track（类型检测）类型没问题时会将字节码换成机器码

<img src="TypeScript.assets/截屏2021-06-05 上午8.14.38.png" alt="截屏2021-06-05 上午8.14.38" style="zoom:50%;" />
...
ts基础数据类型：void，any，never，元祖，枚举，高级类型
...
let arr = new Array<number>(4) // 指定长度
```
...
类型断言（Type Assertion）可以用来手动指定一个值的类型。解决联合类型属性访问时的限制
...
1. type可以声明基本类型，联合类型，元组的别名，interface不行
...
##### 修饰符public private 和 protected

TypeScript 可以使用三种访问修饰符（Access Modifiers），分别是 `public`、`private` 和 `protected`。

- `public` 修饰的属性或方法是公有的，可以在任何地方被访问到，默认所有的属性和方法都是 `public` 的

- `private` 修饰的属性或方法是私有的，不能在声明它的类的外部访问，**使用any还是可以强行访问，可以使用#xxx定义，使用weakMap实现的**
...
Exclude，从一个类型排除另一个类型

Extract，选择可分配给另一种类型的子类型

InstanceType，获取构造类型的实例类型
...
Readonly，让对象中所有属性为只读

ReadonlyArray，制作给定类型的不可变数组

Pick，从符合类型中取出想要的类型组合

Record，从建类型到值类型的映射

Required，让对象中所有属性为必须
