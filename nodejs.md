...
HTTP协议是无状态的，服务器只会响应请求，无法与客户端保持持续连接

HTTP协议是半双工协议，服务器发生的事件无法实时传递给客户端，在同一时刻数据只能单向流动，客户端向服务器发送请求(单向的)，然后服务器响应请求(单向的)。只有等客户端查看状态时才会传递信息（轮询），所以HTTP难以实现实时应用，而且长轮询，长连接都会造成大量的性能浪费

- 单工（半工）：数据单向流动，类似广播，UDP
- 半双工：类似独木桥，HTTP
- 全双工：类似双车道，Websocket
...
- 长轮询是对轮询的改进版，客户端发送HTTP给服务器之后，看有没有新消息，如果没有新消息，就在超时前一直等待
- 当有新消息的时候，才会返回给客户端。在某种程度上减小了网络带宽和CPU利用率等问题。
- 由于http数据包的头部数据量往往很大（通常有400多个字节），但是真正被服务器需要 的数据却很少（有时只有10个字节左右），这样的数据包在网络上周期性的传输，难免对网络带 宽是一种浪费
...
H5的WebSocket协议支持双向通信，实时性更强。

更好的二进制支持。

较少的控制开销。连接创建后，ws客户端、服务端进行数据交换时，协议控制的数据包头部较小。

- 原理：利用HTTP协议握手，HTTP头部中含有WebSocket协议的请求，所以握手成功后利用TCP协议进行交流（QQ）
- 需要浏览器和客户端的支持，Chrome4，firefox4，ie10，safari5；node，apache，nginx1.3
...
WebSocket复用了HTTP的握手通 道。具体指的是，客户端通过HTTP请求与WebSocket服务端协商升级协议。协议升级完成后，后续的数据交换则遵照WebSocket的协议。
        ...
        if (data.match(/Upgrade: websocket/)) {
            ...
            if (headers['Sec-WebSocket-Version'] == 13) {
             ...
             socket.on('data', function (buffers) {
                 ...
                 let _masked = buffers[1] & 0b100000000 === 0b100000000; // 第一位是否是1，是否有掩码 
...
要在html中引入’/socket.io/socket.io.js，然后执行io()函数得到socket对象‘

```html
<script src="/socket.io/socket.io.js" type="text/javascript"></script>
<script>
var socket = io()
socket.emit('自定义事件名', '今天什么日期')
</script>
```

socket对象有emit（发送一个自定义事件）和on方法（监听自定义 事件）

### socket.io框架与express搭配
